...
async def get_dashboard():
    ...
    # 最新评估
    cursor.execute(
        "SELECT * FROM assessment_history ORDER BY assess_date DESC LIMIT 1"
    )
    latest = cursor.fetchone()

    # 评估总次数
    cursor.execute("SELECT COUNT(*) as cnt FROM assessment_history")
    total_count = cursor.fetchone()["cnt"]

    # 报告数量
    cursor.execute("SELECT COUNT(*) as cnt FROM report_history")
    ...
    # 收集所有高风险指标
    high_risk_items = []
    dim_labels = {"customer": "客户风险", "product": "产品/业务风险",
        ...
        for item in dim.get("items", []):
            if item.get("risk") == "高":
                high_risk_items.append({
                    "dimension": dk,
                    "dimension_label": dim_labels.get(dk, dk),
                    "name": item.get("name", ""),
                    "risk": item.get("risk", ""),
